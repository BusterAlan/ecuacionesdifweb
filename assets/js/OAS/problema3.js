...
function paso1Colecta() {
    const massRaw = document.getElementById('masa').value;
    const masa = parseFloat(massRaw);
    ...
    const velocidadI = parseFloat(document.getElementById('velocidadI').value);
...
function paso2Colecta() {
    ...
    const massRaw2 = document.getElementById('masa2').value;
    const masa2 = parseFloat(massRaw2);
    const resultado1 = parseFloat(document.getElementById('resultado1').value);
    const resultadoFinalRaw = document.getElementById('resultado2').value;
    const resultado2 = parseFloat(resultadoFinalRaw);
    ...
    // Validar constante resorte
    if (constanteresorte2 != 12.0) {
        mensajes.push("* La constante de resorte es incorrecta");
    }
    // Validar masa
    if (masa2 != 1.5 && massRaw2 != "1 1/2") {
        mensajes.push("* La masa es incorrecta");
    }
    ...
    // Todo correcto
    if (mensajes.length === 0) {
        mensajes.push("Correcto! | 2√2 ≈ 2.82", true);
    }
    return mensajes;
}
...
function paso2Mensaje(mensajes) {
    ...
    if (mensajes.includes(true)) {
        ...
        mensajeFinal.innerHTML = "Correcto! Terminaste el ejercicio 3";
      } else {
