...
function paso2Colecta() {
    ...
    // Todo correcto
    if (mensajes.length === 0) {
        mensajes.push("Correcto!, esta fórmula nos ayudará a encontrar el pago total por multa. Favor de utilizar la fórmula respecto a t:",true); 
...
function paso3Colecta() {
    ...
    // Todo correcto
    if (mensajes.length === 0) {
        mensajes.push("Correcto!, El pago despúes de 72 años sería 5977.97 centavos",true);
        document.getElementById("centavos").textContent = " ➔ A(72) = 5977.97";
    }
    return mensajes;
}
