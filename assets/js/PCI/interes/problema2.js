...
function paso1Colecta() {
    ...
    if (mensajes.length === 0) {
        ...
        document.getElementById("cIn").textContent = " ➔ C = 6000";
    }
    return mensajes;
}
...
function paso2Colecta() {
    ...
    if (mensajes.length === 0) {
        ...
        var formulaAt = " ➔ <strong>A(t) = 6000e<sup>0.08t</sup></strong>";
...
function paso3Colecta() {
    ...
    // Validar tiempos
    if (tiempoPasado1 !== 20 || tiempoPasado2 !== 20) {
        mensajes.push("* El tiempo es incorrecto");
    }
    // Todo correcto
    if (mensajes.length === 0) {
        mensajes.push("Correcto!, la cantidad a recibir despúes de 20 años sería $29718.19",true);
        document.getElementById("centavos").textContent = " ➔ A(20) = 29718.19";
    }
    return mensajes;
}
